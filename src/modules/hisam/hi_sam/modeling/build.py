...
def build_efficient_sam_vit_s(args):
    return _build_efficient_sam(encoder_patch_embed_dim=384, encoder_num_heads=6, args=args)


def build_efficient_sam_vit_t(args):
    return _build_efficient_sam(encoder_patch_embed_dim=192, encoder_num_heads=3, args=args)
...
def _build_sam(
    ...
    model = HiSam(
        ...
        modal_aligner=ModalAligner(prompt_embed_dim, attn_layers=args.attn_layers, prompt_len=args.prompt_len),
    ...
    if checkpoint is not None:
        ...
        if args.hier_det:
            ...
            if not contain_hi_decoder:
                ...
                with open(os.path.join(args.checkpoint_dir, args.model_type + "_maskdecoder.pth"), "rb") as f:
        ...
        if args.model_type == "vit_b":
            sam_path = os.path.join(args.checkpoint_dir, "sam_vit_b_01ec64.pth")
        elif args.model_type == "vit_l":
            sam_path = os.path.join(args.checkpoint_dir, "sam_vit_l_0b3195.pth")
        elif args.model_type == "vit_h":
            sam_path = os.path.join(args.checkpoint_dir, "sam_vit_h_4b8939.pth")
...
def _build_efficient_sam(encoder_patch_embed_dim, encoder_num_heads, args):
    ...
    if checkpoint is not None:
        ...
        if args.hier_det:
            ...
            if not contain_hi_decoder:
                with open(
                    os.path.join(args.checkpoint_dir, args.model_type + "_maskdecoder.pth"), "rb"
                ) as f_maskdecoder:
