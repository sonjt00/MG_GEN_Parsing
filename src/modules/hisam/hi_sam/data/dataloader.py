    ...
    if training:
        for i in range(len(name_im_gt_list)):
            gos_dataset = OnlineDataset(
                [name_im_gt_list[i]], transform=transforms.Compose(my_transforms), hier_det=hier_det
            )
        ...
        for i in range(len(name_im_gt_list)):
            gos_dataset = OnlineDataset(
                [name_im_gt_list[i]], transform=transforms.Compose(my_transforms), eval_ori_resolution=True
            )
...
class RandomBlur(object):
    def __init__(self, p=0.3, gaussian_kernel_size=[5, 7, 9, 11, 13, 15]):
        ...
        self.gaussian_kernel_size = (
            gaussian_kernel_size  # _setup_size(kernel_size, "Kernel size should be a tuple/list of two integers")
        )
...
class OnlineDataset(Dataset):
    def __init__(self, name_im_gt_list, transform=None, eval_ori_resolution=False, hier_det=False):
        ...
        im_name_list.extend(
            [x.split(os.sep)[-1].split(name_im_gt_list["im_ext"])[0] for x in name_im_gt_list["im_path"]]
        )
    ...
    def __getitem__(self, idx):
        ...
        if "TextSeg" in self.dataset_name:
            gt = (gt_ori >= 127).astype(np.uint8) * 255
        ...
        if self.eval_ori_resolution:
            ...
            if "TextSeg" in self.dataset_name:
                ...
                shape = sample["ignore_mask"].shape
                sample["ignore_mask"] = sample["ignore_mask"][..., -1:].reshape(*(shape[:-1]))
...
train_transforms = [ColorJitter(), RandomRotate(), LargeScaleJitter(), ToTensor()]  # image: np.uint8->np.float32
